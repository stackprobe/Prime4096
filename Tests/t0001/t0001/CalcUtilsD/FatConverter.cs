	...
	public class FatConverter
	{
		...
		private void Normalize()
		{
			...
			if (start < this.Figures.Count)
			{
				...
				{
					this.Figures = new List<int>(CalcDSysUtils.CopyOfRange(this.Figures.ToArray(), start, end));
					this.Exponent += start;
				}
			}
			else
		...
		public FatFloat GetFloat()
		{
			...
			// zantei
			{
				...
				this.Figures = new List<int>(CalcDSysUtils.Shift(this.Figures.ToArray(), a));
				this.Exponent -= a;
				...
				this.Figures = new List<int>(CalcDSysUtils.CopyOf(this.Figures.ToArray(), this.Figures.Count + z));
			}
